    ...
    do JW = 1, NWB
        ...
        do JB = BS(JW), BE(JW)
            ...
            do SegNumI = IU, ID
                ...
                if (.not. BedConsolidationSeg(SegNumI)) then
                    cycle
                end if ! cb 6/28/18
...
!Get Erosion/Deposition
    if (CEMASedimentProcessesInc) then
        call CEMASedimentProcesses()
    end if
    ...
    do JW = 1, NWB
        ...
        do JB = BS(JW), BE(JW)
            ...
            do SegNumI = IU, ID
                ...
                do K = KT, KB(SegNumI)
                    if (K == KT) then
                        VolumePresent = VolumePresent + B(K, SegNumI)*DLX(SegNumI)*(H(K, JW) - Z(SegNumI))
                    end if
                    if (K /= KT) then
                        VolumePresent = VolumePresent + B(K, SegNumI)*DLX(SegNumI)*H(K, JW)
                    end if
                end do !K
            end do !SegNumI
        end do !JB
    ...
    do JW = 1, NWB
        ...
        do JB = BS(JW), BE(JW)
            ...
            do SegNumI = IU, ID
                ...
                do K = KT, KB(SegNumI)
                    if (K == KT) then
                        VolumeNew = VolumeNew + B(K, SegNumI)*DLX(SegNumI)*(H(K, JW) - Z(SegNumI))
                    end if
                    if (K /= KT) then
                        VolumeNew = VolumeNew + B(K, SegNumI)*DLX(SegNumI)*H(K, JW)
                    end if
                end do !K
            end do !SegNumI
        end do !JB
        ...
        do JW = 1, NWB
            ...
            do JB = BS(JW), BE(JW)
                ...
                do SegNumI = IU, ID
                    ...
                    do K = KT, KB(SegNumI)
                        if (K == KT) then
                            VolumeUpdated = VolumeUpdated + B(K, SegNumI)*DLX(SegNumI)*(H(K, JW) - Z(SegNumI))
                        end if
                        if (K /= KT) then
                            VolumeUpdated = VolumeUpdated + B(K, SegNumI)*DLX(SegNumI)*H(K, JW)
                        end if
                    end do !K
                end do !SegNumI
            end do !JB
        ...
        do JW = 1, NWB
            do JB = BS(JW), BE(JW)
                do I = CUS(JB) - 1, DS(JB)
                    ...
                    if (kbi(i) < kb(i)) then
                        depthb(ktwb(jw), i) = h1(ktwb(jw), i) - (el(kbi(i) + 1, i) - el(kb(i) + 1, i))
                    end if ! SW 1/23/06
...
!SP added to test 04/01/2010
    ...
    if (BottomUpdated .and. MoveFFTLayerDown) then
        call MoveFFTLayerConsolid()
    end if
...
!New sediment concentration
    ...
    return
end subroutine CEMASedimentProcesses
...
!No Erosion
    ...
    if (CEMASedimentType == 2) then !Non-Cohesive Sediments
        ...
        if (YALINP <= 100.) then
            ...
            if (YALINP > 100. .and. YALINP <= 3000.) then
...
!If(FRIC(I) /= 0.0) GC2 = G/(FRIC(I)*FRIC(I))
            if (FRIC(SegNumI) /= 0.0) then
                GC2 = G/(FRIC(SegNumI)*FRIC(SegNumI))
            end if !SW 3/2019
        end if
        Usr = Sqrt(GC2)*UctAvg
!
!Erosion process
        ...
        if (Tsks > 0.0) then
            Caks = 0.015*Dks/rgha*Tsks**1.5/Dsks**0.3*SRho*1000.0
        end if !gm/m^3
!
...
!Erosion Equation 12 Page 9
        if (Caks > Capda) then
            SScour = (-BetaD)*SDfz*(Capda - Caks)/(rghapda - rgha)
        end if

    end if

    return
end subroutine CEMACalculateScour
