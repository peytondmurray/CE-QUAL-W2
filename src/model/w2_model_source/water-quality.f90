...
! enhanced pH buffering end
    ...
    character(len=2) :: MIGRATION ! CO 6/3/2019
    save
...
!INQUIRE(FILE='w2_AlgaeMigration.csv',EXIST=ALGAE_SETTLING_EXIST)    
    if (ALGAE_SETTLING_EXIST) then
        ...
        read(2450, *)
        allocate(MIGRATE_GROUP(NMIG), MIGRATE_MODEL(NMIG), AMP(NMIG), PHASE(NMIG), C_COEFF_EXT(NMIG), RAD(NMIG), MIND(NMIG), MAXD(NMIG), DENSI(NMIG), DENBI(NMIG), T_DEC(NMIG), TS_DEC(NMIG), C_DENINC(NMIG), C_DENDEC(NMIG), DEPTH_LIM_ONOFF(NMIG), DEPTH_LIM(NMIG), LOSS_FRAC(NMIG), I_C(NMIG), C_DENINC_1(NMIG), C_DENINC_2(NMIG), C_DENDEC_1(NMIG), C_DENDEC_2(NMIG), DENP_MINS(NMIG), DENP_MINB(NMIG), DEN_COR(MIGI), NMINT(NMIG), MIGON(NMIG, 12), MIGOFF(NMIG, 12), DEPTH_CALC_ONOFF(NMIG), EXP_DEPTH(NMIG))
        do I = 1, NMIG
            ...
            if (MIGRATE_MODEL(I) == 1 .or. MIGRATE_MODEL(I) == 2) then
                ...
                if (MIGRATE_MODEL(I) == 3) then
                    ...
                    read(2450, *)
                    read(2450, *)
                    read(2450, *) RAD(I), MIND(I), MAXD(I), DENSI(I), DENBI(I), I_C(I), C_DENINC_1(I), C_DENINC_2(I), C_DENDEC_1(I), C_DENDEC_2(I), DENP_MINS(I), DENP_MINB(I), DEN_COR(I), DEPTH_LIM_ONOFF(I), DEPTH_LIM(I), LOSS_FRAC(I)
                    read(2450, *)
                end if
            end if
            ALGAE_SETTLING(MIGRATE_GROUP(I)) = .true. ! CO 6/5/2019
        end do
        allocate(DEN_AVG(KMX, IMX, NMIG), DEN(KMX, IMX, MAXVAL(TS_DEC), NMIG), DEN1(KMX, IMX, NMIG), DEN2(KMX, IMX, NMIG), DENP(KMX, IMX, NMIG), DENP_MIN(KMX), ASETTLE(KMX, IMX, NAL), ALLIM_OLD(KMX, IMX, NMIG), TWQ(MAXVAL(TS_DEC), NMIG), LOLD(IMX, NMIG))
        100 close(2450)
    end if
            ...
            if (ZOOPLANKTON_CALC) then
                do JZ = 1, NZP
                    ...
                    if (TGRAZE(K, I, JZ) <= 0.0 .or. O2(K, I) < 2.0) then
                        ...
                        do JA = 1, NAL
                            if (ALG_CALC(JA)) then
                                AGZ(K, I, JA, JZ) = ZMU(K, I, JZ)*ZOO(K, I, JZ)*ALG(K, I, JA)*PREFA(JA, JZ)/TGRAZE(K, I, JZ)
                            end if !  KV 5/26/2007
                        end do
...
! Algal rates
    if (ALGAE_SETTLING_EXIST .and. jday > iday) then
        NITWQ = NITWQ + 1
    end if
    do JA = 1, NAL
        if (ALG_CALC(JA)) then
...
!****** Limiting factor
                    ...
                    if (AHSP(JA) /= 0.0 .and. po4_calc) then
                        APLIM(K, I, JA) = FDPO4*PO4(K, I)/(FDPO4*PO4(K, I) + AHSP(JA) + NONZERO)
                    end if ! cb 10/12/11
                    if (AHSN(JA) /= 0.0 .and. n_calc) then
                        ANLIM(K, I, JA) = (NH4(K, I) + NO3(K, I))/(NH4(K, I) + NO3(K, I) + AHSN(JA) + NONZERO)
                    end if ! cb 10/12/11
                    if (AHSSI(JA) /= 0.0 .and. DSI_CALC) then
                        ASLIM(K, I, JA) = DSI(K, I)/(DSI(K, I) + AHSSI(JA) + NONZERO)
                    end if ! cb 10/12/11
...
!****** Algal rates
                    ...
                    if (ALGAE_SETTLING(JA) .and. ISETTLE == 1) then
                        if (MIGRATE_MODEL(MIGI) == 1) then !   TIME VARYING VELOCTY
                            ...
                            if (ALGMIGRATION_DEBUG == 1) then
                                write(2451, '("Model1:,",3(I3,","),3(F15.5,","))') K, I, JA, JDAY, ASETTLE(K, I, JA)
                            end if
                        else
                            if (MIGRATE_MODEL(MIGI) == 2) then !  TIME AND SPACE VARYING VELOCITY
                                ...
                                if (ALGMIGRATION_DEBUG == 1) then
                                    write(2451, '("Model2:,",3(I3,","),3(F15.5,","))') K, I, JA, JDAY, ASETTLE(K, I, JA)
                                end if
                            else
                                if (MIGRATE_MODEL(MIGI) == 3) then ! DENSITY CHANGE VELOCITY
                                    ...
                                    if (NITWQ == 1) then ! SET INITIAL DENSITY
                                        ...
                                        if (ALGMIGRATION_DEBUG == 1) then
                                            write(2451, '("Model3:,",4(I6,","),4(F15.5,","))') K, I, JA, nitwq, JDAY, ASETTLE(K, I, JA), DEN_avg(K, I, MIGI), den(k, i, min(nitwq, ts_dec(migi)), migi)
                                        end if
                                    else
                                        if (NITWQ <= TS_DEC(MIGI)) then ! STORE ALL DENSITY VALUES UNTIL MAXIMUM NUMBER IS REACHED
                                    ...
                                    if (NITWQ == 1) then ! SET INITIAL DENSITY
                                        ...
                                        if (abs(asettle(k, i, ja)) > 1000.) then
                                            asettle(k, 1, ja) = 0.0
                                        end if
                                        DENP(K, I, MIGI) = DENP_MIN(K)
                                        if (ALGMIGRATION_DEBUG == 1) then
                                            write(2451, '("Model4:,",4(I6,","),3(F15.5,","))') K, I, JA, NITWQ, JDAY, ASETTLE(K, I, JA), DEN1(K, I, MIGI)
                                        end if
                                        if (k == kb(i) .and. i == id .and. jb == nbr .and. jw == nwb) then
                                            TOLD = JDAY
                                        end if
                                        if (K == KB(I)) then
                                            LOLD(I, MIGI) = LIGHT*ASAT(JA)
                                        end if
                                    else
                                        ...
                                        if (ALGMIGRATION_DEBUG == 1) then
                                            write(2451, '("Model4:,",4(I6,","),3(F15.5,","))') K, I, JA, NITWQ, JDAY, ASETTLE(K, I, JA), DEN2(K, I, MIGI)
                                        end if
                                        if (k == kb(i) .and. i == id .and. jb == nbr .and. jw == nwb) then
                                            TOLD = JDAY
                                        end if
                        ...
                        if (depth_lim_onoff(migi) == 1 .and. depthb(k, i) < depth_lim(migi)) then
                            asettle(k, i, ja) = 0
                        end if
...
!TS    = 2.0*PI*U2/G*1.2*  TANH(COEF3)*TANH(COEF4/TANH(COEF3))
            ...
            TAU = 0.003*UORB*UORB
            if (TAU - TAUCR(J) > 0.0) then
                EPSILON = MAX(0.0, 0.008/49.0*(TAU - TAUCR(J))**3*10000.0/DLT)
            end if
...
! PO4SD(K,I)  = SEDDp(K,I)+sedd1(k,i)*orgp(jw) + sedd2(k,i)*orgp(jw)   ! Amaila
            ...
            PO4SS(K, I) = PO4AP(K, I) + PO4EP(K, I) + PO4OM(K, I) + PO4SD(K, I) + PO4SR(K, I) + PO4NS(K, I) + PO4BOD(K, I) + PO4MR(K, I) - PO4MG(K, I) + PO4ZR(K, I)
...
!        IF(BOD_CALC(JCB))NH4BOD(K,I) =  NH4BOD(K,I)+CBODD(K,I,JCB)*CBOD(K,I,JCB)*BODN(JCB)
            ...
            do JA = 1, NAL
                if (ALG_CALC(JA)) then
                    if (ANEQN(JA) == 2) then
                        NH4PR = NH4(K, I)*NO3(K, I)/((ANPR(JA) + NH4(K, I))*(ANPR(JA) + NO3(K, I))) + NH4(K, I)*ANPR(JA)/((NO3(K, I) + NH4(K, I) + NONZERO)*(ANPR(JA) + NO3(K, I)))
                    else
                        NH4PR = NH4(K, I)/(NH4(K, I) + NO3(K, I) + NONZERO)
                    end if
                    if (AHSN(JA) > 0.0) then
                        NH4AG(K, I) = NH4AG(K, I) + AGR(K, I, JA)*ALG(K, I, JA)*AN(JA)*NH4PR
                    end if
            ...
            do JE = 1, NEP
                if (EPIPHYTON_CALC(JW, JE)) then
                    if (ENEQN(JE) == 2) then
                        NH4PR = NH4(K, I)*NO3(K, I)/((ENPR(JE) + NH4(K, I))*(ENPR(JE) + NO3(K, I))) + NH4(K, I)*ENPR(JE)/((NO3(K, I) + NH4(K, I) + NONZERO)*(ENPR(JE) + NO3(K, I)))
                    else
                        NH4PR = NH4(K, I)/(NH4(K, I) + NO3(K, I) + NONZERO)
                    end if
                    if (EHSN(JE) > 0.0) then
                        NH4EG(K, I) = NH4EG(K, I) + EGR(K, I, JE)*EPC(K, I, JE)*EN(JE)*NH4PR
                    end if
...
!NH4SD(K,I)  =  SEDDn(K,I) +sedd1(k,i)*orgn(jw) + sedd2(k,i)*orgn(jw)   ! Amaila
            ...
            NH4SS(K, I) = NH4AP(K, I) + NH4EP(K, I) + NH4OM(K, I) + NH4SD(K, I) + NH4SR(K, I) + NH4BOD(K, I) - NH4D(K, I) + NH4MR(K, I) - NH4MG(K, I) + NH4ZR(K, I)
            if (K == KT .and. CDWBC(NH3_DER, JW) == "      ON") then
                NH4SS(K, I) = NH4SS(K, I) - NH3GAS(K, I)
            end if ! NH3 VOLATILIZATION IF PH IS ON AND NH3 DER ON
        end do
    end do
    return
        ...
        do K = KT, KB(I)
            do JA = 1, NAL
                if (ALG_CALC(JA)) then
                    ...
                    if (ANEQN(JA) == 2) then
                        NO3PR = 1.0 - (NH4(K, I)*NO3(K, I)/((ANPR(JA) + NH4(K, I))*(ANPR(JA) + NO3(K, I))) + NH4(K, I)*ANPR(JA)/((NO3(K, I) + NH4(K, I) + NONZERO)*(ANPR(JA) + NO3(K, I))))
                    end if
                    if (AHSN(JA) > 0.0) then
                        NO3AG(K, I) = NO3AG(K, I) + AGR(K, I, JA)*ALG(K, I, JA)*NO3PR*AN(JA)
                    end if
                end if
            end do
            do JE = 1, NEP
                if (EPIPHYTON_CALC(JW, JE)) then
                    NO3PR = 1.0 - NH4(K, I)/(NH4(K, I) + NO3(K, I) + NONZERO)
                    if (ENEQN(JE) == 2) then
                        NO3PR = 1.0 - (NH4(K, I)*NO3(K, I)/((ENPR(JE) + NH4(K, I))*(ENPR(JE) + NO3(K, I))) + NH4(K, I)*ENPR(JE)/((NO3(K, I) + NH4(K, I) + NONZERO)*(ENPR(JE) + NO3(K, I))))
                    end if
                    if (EHSN(JE) > 0.0) then
                        NO3EG(K, I) = NO3EG(K, I) + EGR(K, I, JE)*EPC(K, I, JE)*NO3PR*EN(JE)
                    end if
                end if
            end do
            if (K == KB(I)) then ! SW 4/18/07
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        do K = KT, KB(I)
            ...
            do JE = 1, NEP
                if (EPIPHYTON_CALC(JW, JE)) then
                    PSIEM(K, I) = PSIEM(K, I) + EMR(K, I, JE)*EPC(K, I, JE)*ESI(JE)
                end if
            end do
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        do K = KT, KB(I)
            ...
            if (STANDING_BIOMASS_DECAY) then ! SW 5/26/15
                TICSS(K, I) = TICAP(K, I) + TICEP(K, I) + SEDDC(K, I) + ORGC(JW)*(LPOMD(K, I) + RPOMD(K, I) + LDOMD(K, I) + RDOMD(K, I)) + CO2R(JW)*SODD(K, I)*DO3(K, I) + TICBOD(K, I) + TICMC(K, I) + TICZR(K, I) + ticch4 + sedd1(k, i)*cbiom(jw) + sedd2(k, i)*cbiom(jw) ! Amaila, cb 6/7/17

            else
                TICSS(K, I) = TICAP(K, I) + TICEP(K, I) + SEDDC(K, I) + ORGC(JW)*(LPOMD(K, I) + RPOMD(K, I) + LDOMD(K, I) + RDOMD(K, I)) + CO2R(JW)*SODD(K, I)*DO3(K, I) + TICBOD(K, I) + TICMC(K, I) + TICZR(K, I) + ticch4 !!8/2020   
            end if
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        ...
        do K = KT, KB(I)
            ...
            do JA = 1, NAL
                if (ALG_CALC(JA)) then
                    SEDAS(K, I) = SEDAS(K, I) + MAX(AS(JA), 0.0)*ALG(K, I, JA)*BIBH2(K, I)
                end if !BI(K,I)/BH2(K,I)*(1.0-BI(K+1,I)/BI(K,I))
            end do
            SEDEM = 0.0 ! CB 5/19/06
            do JE = 1, NEP
!        LPOMEP(K,I) = LPOMEP(K,I)+EPOM(JE)*(EMR(K,I,JE)*EPC(K,I,JE))
                if (EPIPHYTON_CALC(JW, JE)) then
                    SEDEM = SEDEM + EBR(K, I, JE)*EPC(K, I, JE)
                end if ! SW 3/2019 SEDEM = SEDEM+EBR(K,I,JE)/H1(K,I)*EPC(K,I,JE)    ! cb 5/19/06     
            end do
            do JD = 1, NBOD
                if (BOD_CALC(JD)) then
                    SEDCB(K, I) = SEDCB(K, I) + MAX(CBODS(JD), 0.0)*CBOD(K, I, JD)*BIBH2(K, I)/O2OM(JW)
                end if !BI(K,I)/BH2(K,I)*(1.0-BI(K+1,I)/BI(K,I))
            end do
!
            if (ORGC_CALC) then
...
!SEDP(K,I)    = MAX(SEDP(K,I)+(LPOMEPP(K,I)+SEDASP(K,I)+SEDOMSP(K,I)+SEDCBP(K,I)+SEDNSP(K,I)-SEDDP(K,I)   &   ! SW 4/8/16
                SEDP(K, I) = MAX(SEDP(K, I) + SEDPINFLUX(K, I) + (SEDNSP(K, I) - SEDDP(K, I) - SEDBRP(K, I))*DLT, 0.0) !cb 11/30/06
            else
                if (k == kb(i)) then
...
! CEMA start
            ...
            if (k < kb(i)) then
                SEDN(K, I) = MAX(SEDN(K, I) + SEDNINFLUX(K, I) + (SEDNSN(K, I) + SEDNO3(K, I) - SEDDN(K, I) - SEDBRN(K, I))*DLT, 0.0) !CB 11/30/06                    
            else
                if (k == kb(i)) then
                    SEDN(K, I) = MAX(SEDN(K, I) + SEDNINFLUX(K, I) + (SEDNSN(K, I) + SEDNO3(K, I) - SEDDN(K, I) - SEDBRN(K, I))*DLT, 0.0) !CB 11/30/06
                end if
...
! CEMA start
            if (k < kb(i)) then
                SEDC(K, I) = MAX(SEDC(K, I) + (SEDEM + SEDASC(K, I) + SEDOMSC(K, I) + SEDCBC(K, I) + SEDNSC(K, I) - SEDDC(K, I) - SEDBRC(K, I))*DLT, 0.0)
            else
                if (k == kb(i) .and. .not. sediment_diagenesis) then
                    SEDC(K, I) = MAX(SEDC(K, I) + (SEDEM + SEDASC(K, I) + SEDOMSC(K, I) + SEDCBC(K, I) + SEDNSC(K, I) - SEDDC(K, I) - SEDBRC(K, I))*DLT, 0.0)
                end if
            end if
! CEMA end
...
!**** Limiting factor
            ...
            if (EHSP(J) /= 0.0) then
                EPLIM(K, I, J) = FDPO4*PO4(K, I)/(FDPO4*PO4(K, I) + EHSP(J) + NONZERO)
            end if
            if (EHSN(J) /= 0.0) then
                ENLIM(K, I, J) = (NH4(K, I) + NO3(K, I))/(NH4(K, I) + NO3(K, I) + EHSN(J) + NONZERO)
            end if
            if (EHSSI(J) /= 0.0) then
                ESLIM(K, I, J) = DSI(K, I)/(DSI(K, I) + EHSSI(J) + NONZERO)
            end if
...
!**** Sources/sinks
            ...
            EGR(K, I, J) = MIN(ETRM(K, I, J)*EG(J)*LIMIT*BLIM, PO4(K, I)/(EP(J)*DLT*EPD(K, I, J)/H1(KT, I) + NONZERO), (NH4(K, I) + NO3(K, I))/(EN(J)*DLT*EPD(K, I, J)/H1(K, I) + NONZERO))
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        do K = KT, KB(I)
            ...
            LPOMNSS(K, I) = LPOMNAP(K, I) + LPOMNEP(K, I) + LPOMNMP(K, I) - LPOMND(K, I) + LPOMNNS(K, I) - LRPOMND(K, I) - LPOMNHD(K, I) + LPZOOOUTN(K, I) - LPZOOINN(K, I)
        end do
    end do
    return
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        do K = KT, KB(I)
            do JA = 1, NAL
                if (ALG_CALC(JA)) then
                    LDOMCAP(K, I) = LDOMCAP(K, I) + (AER(K, I, JA) + (1.0 - APOM(JA))*AMR(K, I, JA))*ALG(K, I, JA)*AC(JA)
                end if
            end do
            do JE = 1, NEP
                if (EPIPHYTON_CALC(JW, JE)) then
                    LDOMCEP(K, I) = LDOMCEP(K, I) + (EER(K, I, JE) + (1.0 - EPOM(JE))*EMR(K, I, JE))*EPC(K, I, JE)*EC(JE)
                end if
            end do
            do M = 1, NMC
                if (MACROPHYTE_CALC(JW, M)) then
...
!***********************************************************************************************************************************
    ...
    do I = IU, ID
        do K = KT, KB(I)
            do JA = 1, NAL
                if (ALG_CALC(JA)) then
                    LPOMCAP(K, I) = LPOMCAP(K, I) + APOM(JA)*AMR(K, I, JA)*ALG(K, I, JA)*AC(JA)
                end if
            end do
            do JE = 1, NEP
                if (EPIPHYTON_CALC(JW, JE)) then
                    LPOMCEP(K, I) = LPOMCEP(K, I) + EPOM(JE)*EMR(K, I, JE)*EPC(K, I, JE)*EC(JE)
                end if
            end do
            do M = 1, NMC
                if (MACROPHYTE_CALC(JW, M)) then
            ...
            LPOMCSS(K, I) = LPOMCAP(K, I) + LPOMCEP(K, I) + LPOMCMP(K, I) - LPOMCD(K, I) + LPOMCNS(K, I) - LRPOMCD(K, I) - LPOMCHD(K, I) + LPZOOOUTC(K, I) - LPZOOINC(K, I)
        end do
    end do
    return
...
!COLDEP=ELWS(I)-COLB
            ...
            if (MACRC(JJ, KT, I, M) > MMAX(M)) then
                MGR(JJ, KT, I, M) = 0.0
            end if
        ...
        do K = KT + 1, KB(I)
            ...
            JE = KB(I)
            do JJ = JT, JE
                if (MACRC(JJ, K, I, M) > MMAX(M)) then
                    MGR(JJ, K, I, M) = 0.0
                end if
                MACSS(JJ, K, I, M) = (MGR(JJ, K, I, M) - MMR(K, I, M) - MRR(K, I, M))*MACRC(JJ, K, I, M)
                if (MACT(JJ, K, I) > MBMP(M) .and. MACT(JJ, K - 1, I) < MBMP(M) .and. MACSS(JJ, K, I, M) > 0.0) then
                    if (K - 1 == KT) then
        ...
        do JB = BS(JW), BE(JW) ! SW 3/9/16
            do I = CUS(JB), DS(JB)
                do K = KT, KB(I)
                    ...
                    if (K == KT .and. CDWBC(NH3_DER, JW) == "      ON") then
                        NH3GASLOSS(JW) = NH3GASLOSS(JW) + NH3GAS(K, I)*VOL(K, I)*DLT13
                    end if ! NH3GAS in g/m3/s, convert to kg, cumulative      
                end do
            end do
        end do
    end if
...
!**** Ionic strength
            ...
            if (SALT_WATER(JW)) then
                S2 = 1.47E-3 + 1.9885E-2*TDS(K, I) + 3.8E-5*TDS(K, I)*TDS(K, I)
            end if
...
!**** pH, carbon dioxide, bicarbonate, and carbonate concentrations
            ...
            if (CDWBC(HCO3_DER, JW) == "      ON") then
                HCO3(K, I) = TIC(K, I)/(1.0 + HION/K1 + K2/HION)
            end if ! mg/l as C
            if (CDWBC(CO3_DER, JW) == "      ON") then
...
! pH and carbonate species
    do I = IU, ID
        do K = KT, KB(I)
            ...
            if (POM_BUFFERING) then
                OMCT = OMCT + (LPOM(K, I) + RPOM(K, I))*ORGC(JW)/12011.
            end if ! SR 01/01/12
!**** Ionic strength
            if (FRESH_WATER(JW)) then
                S2 = 2.5E-05*TDS(K, I)
            ...
            do N = 1, 3
                ...
                do while (F > 0.0 .and. ITER < 12)
                    ...
                    if (PHOSPHATE_BUFFERING) then ! SR 01/01/12
                        F = F + PHOST*(KP1*KP2*HION + 2*KP1*KP2*KP3 - HION*HION*HION)/(HION*HION*HION + KP1*HION*HION + KP1*KP2*HION + KP1*KP2*KP3) ! SR 01/01/12
                    end if ! SR 01/01/12
...
!BODTOT = BODTOT+CBOD(K,I,IBOD)
                            ...
                            if (CBODS(IBOD) > 0.0) then
                                TOTSS(K, I) = TOTSS(K, I) + CBOD(K, I, IBOD)/O2OM(JW)
                            end if ! SW 9/5/13  Added particulate CBOD to TSS computation
                        end if
                    end do
                    if (ORGC_CALC) then
...
!END DO
                    ...
                    if (CDWBC(TURB_DER, JW) == "      ON") then
                        TURB(K, I) = EXP(CoeffA_Turb(JW)*LOG(TOTSS(K, I)) + CoeffB_Turb(JW))
                    end if
                    if (CDWBC(SECCHI_DER, JW) == "      ON") then
                        SECCHID(K, I) = SECC_PAR(JW)/GAMMA(K, I)
                    end if ! Secchi Disk
...
! Nitrogen has an atomic mass of 14.00674. These numbers account for the factor of 50.044/14.00674 used below.
    ...
    do I = IU, ID
        do K = KT, KB(I)
            if (noncon_alkalinity) then
                ALKSS(K, I) = 50.044/14.00674*(14./16.*(NH4AP(K, I) + NH4EP(K, I) + NH4ZR(K, I) + NH4MR(K, I) - NH4MG(K, I)) + 18./16.*(NO3AG(K, I) + NO3EG(K, I)) - 2.*NH4D(K, I) + NO3D(K, I) + NO3SED(K, I)*(1 - FNO3SED(JW)))
            else
    ...
    if (MIGRATION == "ON") then
        deallocate(ASETTLE, DEN_AVG, DENP, DEN1, DEN2, ALLIM_OLD, DEN, AMP, PHASE, C_COEFF_EXT, RAD, MIND, MAXD, DENSI, DENBI, T_DEC, C_DENINC, C_DENDEC, DEPTH_LIM, LOSS_FRAC, TWQ, I_C, C_DENINC_1, C_DENINC_2, C_DENDEC_1, C_DENDEC_2, DENP_MINS, DENP_MINB, DENP_MIN, DEN_COR, MIGRATE_GROUP, MIGRATE_MODEL, TS_DEC, DEPTH_LIM_ONOFF, NMINT, MIGON, MIGOFF, LOLD, DEPTH_CALC_ONOFF, EXP_DEPTH)
    end if ! CO 6/12/2019
    return
