...
contains
    ...
    subroutine InitCond_SedFlux()
        implicit none
        character(len=256) :: ParameterDesc
...
!
    ...
    end subroutine InitCond_SedFlux
...
!
                if (IncludeAlkalinity) then
                    SD_ALK0 = MAX(C2(LayerNum, SegNumI, NALK), 0.0)
                end if
!
                if (IncludeIron) then
...
!Update source/sink terms
                ...
                if (IncludeIron) then
                    FEIISS(LayerNum, SegNumI) = FEIISS(LayerNum, SegNumI) + Dissolved_Fe2_Src
                end if !Fe(II)
                if (IncludeManganese) then
                    MNIISS(LayerNum, SegNumI) = MNIISS(LayerNum, SegNumI) + Dissolved_Mn2_Src
                end if !Mn(II)
                TICSS(LayerNum, SegNumI) = TICSS(LayerNum, SegNumI) + Dissolved_CO2_Src !CO2
                if (IncludeAlkalinity) then
                    ALKSS(LayerNum, SegNumI) = ALKSS(LayerNum, SegNumI) + Dissolved_ALK_Src
                end if !ALkalinity
...
!
        ...
        if (FirstTimeInBubbles) then
            FirstTimeInBubbles = .false.
        end if

    end subroutine SedimentFlux
...
!
            if (IncludeIron) then
                SD_JFeOOHin = SdinFeOOH(LayerNum, Segnumi)
            end if
            if (IncludeManganese) then
                SD_JMnO2in = SdinMnO2(LayerNum, Segnumi)
            end if
!
...
!
            if (IncludeIron) then
                SD_JinFeOOH = SD_JinFeOOH*DAY
            end if
            if (IncludeManganese) then
                SD_JMnO2in = SD_JMnO2in*DAY
            end if
!
...
!
            ...
            SD_EPOP = 0.0
            do iTemp = 1, 3
                if (SD_POM > NONZERO .and. SD_POCT2 > Nonzero) then
                    SD_EPOC(itemp) = SD_POCT2/SD_POM*SD_POC22(itemp)/SD_POCT2*SD_E
                end if
                if (SD_POM > NONZERO .and. SD_PONT2 > Nonzero) then
                    SD_EPON(itemp) = SD_PONT2/SD_POM*SD_PON22(itemp)/SD_PONT2*SD_E
                end if
                if (SD_POM > NONZERO .and. SD_POPT2 > Nonzero) then
                    SD_EPOP(itemp) = SD_POPT2/SD_POM*SD_POP22(itemp)/SD_POPT2*SD_E
                end if
            end do
        end if
!
...
!Equation 13.32 DiToro. See also Equation 12.2 - also includes resuspension
        do iTemp = 1, 3
            SD_POC22(iTemp) = (SD_POC22(iTemp) + SD_JPOC(iTemp)*SD_tc/SD_H2 - SD_EPOC(iTemp)*SD_tc/SD_H2)/(1. + SD_kdiaPOC(iTemp)*SD_ThtaPOC(iTemp)**(SD_T2 - 20.)*SD_tc + SD_W2*SD_tc/SD_H2)
            SD_PON22(iTemp) = (SD_PON22(iTemp) + SD_JPON(iTemp)*SD_tc/SD_H2 - SD_EPON(iTemp)*SD_tc/SD_H2)/(1. + SD_kdiaPON(iTemp)*SD_ThtaPON(iTemp)**(SD_T2 - 20.)*SD_tc + SD_W2*SD_tc/SD_H2)
            SD_POP22(iTemp) = (SD_POP22(iTemp) + SD_JPOP(iTemp)*SD_tc/SD_H2 - SD_EPOP(iTemp)*SD_tc/SD_H2)/(1. + SD_kdiaPOP(iTemp)*SD_ThtaPOP(iTemp)**(SD_T2 - 20.)*SD_tc + SD_W2*SD_tc/SD_H2)
            if (isnan(SD_POC22(iTemp))) then
                SD_POC22(iTemp) = 0.0
            end if
            if (isnan(SD_PON22(iTemp))) then
                SD_PON22(iTemp) = 0.0
            end if
            if (isnan(SD_POP22(iTemp))) then
                SD_POP22(iTemp) = 0.0
            end if
...
!
    ...
    end subroutine CEMAMFTRatesandConstants
...
!
        if (BUBBLES_CALCULATION) then
            call GasBubblesFormation(BubbleRadiusSed(SegNumI), sd_tc, VolWater)
        end if
!
!2
!Porewater release
...
!Diffusive flux of NH3, NO3, CH4, SO4, H2S, CO2
        ...
        if (IncludeAlkalinity) then
            Dissolved_Alk_Src = Dissolved_ALK_Src + SD_JALK*BIBH22
        end if
        Dissolved_PO4_Src = Dissolved_PO4_Src + SD_JPO4*BIBH22
        if (IncludeIron) then
            Dissolved_Fe2_Src = Dissolved_Fe2_Src + SD_JFe2*BIBH22
        end if
        if (IncludeManganese) then
            Dissolved_Mn2_Src = Dissolved_Mn2_Src + SD_JMn2*BIBH22
        end if
!
...
! resuspension of POM, POP, and PON
    ...
    end subroutine SedimentReaction
...
!**** Ionic strength
        ...
        if (SALT_WATER(JW)) then
            S2 = 1.47E-3 + 1.9885E-2*TDSsed + 3.8E-5*TDSsed*TDSsed
        end if
        ...
        do N = 1, 3
            ...
            do while (F1 > 0.0 .and. ITER1 < 12)
                ...
                if (PHOSPHATE_BUFFERING) then ! SR 01/01/12
                    F1 = F1 + PHOST*(KP1*KP2*HION + 2*KP1*KP2*KP3 - HION*HION*HION)/(HION*HION*HION + KP1*HION*HION + KP1*KP2*HION + KP1*KP2*KP3) ! SR 01/01/12
                end if ! SR 01/01/12
...
!**** pH, carbon dioxide, bicarbonate, and carbonate concentrations
    ...
    end subroutine PH_SEDIMENTS


    subroutine CEMAWindInducedSedimentResuspension()
        epsilon = 0.0
        FETCHW = FETCHD(SegNumI, JB)
        if (COS(PHI(JW) - PHI0(SegNumI)) < 0.0) then
            FETCHW = FETCHU(SegNumI, JB)
        end if
...
!TS    = 2.0*PI*U2/G*1.2*  TANH(COEF3)*TANH(COEF4/TANH(COEF3))
        ...
        TAU = 0.003*UORB*UORB
        if (TAU - TAUCRPOM > 0.0) then
            EPSILON = MAX(0.0, 0.008/49.0*(TAU - TAUCRPOM)**3*10000.0/sd_tc)
        end if
        SD_E = EPSILON*DLX(SegNumI)*BI(LayerNum, SegNumI)/VOL(LayerNum, SegNumI) ! SD_E: g/m^2/s
    end subroutine CEMAWindInducedSedimentResuspension
    ...
    end subroutine CEMABottomScourResuspension
