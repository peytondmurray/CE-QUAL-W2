...
!DEC$ATTRIBUTES STDCALL :: CE_QUAL_W2
    ...
    if (STOP_PUSHED) then
        RESULT = DLGSET(DLG, RESTART, .true., DLG_ENABLE)
    end if !Enable  'Restart' button
...
subroutine SCREEN_UPDATE(DLG)
    ...
    end if;     if (MINDLT < 1.0) then
        write(TEXT1, "(F0.3)") MINDLT
    end if
