...
MODULE AZ_LOCAL
 ...
 use macrophytec
 use LOGICC
 USE MAIN, ONLY:WARNING_OPEN, IUT,IDT
 ...
 USE MAIN, ONLY:WARNING_OPEN
  ...
  ELSE
    ...
    DO K=KT,KBMIN(I)-1
      ...
      RIAZ0   =  LOG(AZ0/AZMAX(JW))*0.666666667D0                                                  ! /1.5
...
!CALCULATE HORIZONTAL PORTION OF THE SPLIT K-E EQUATIONS FOR INTERIOR LAYERS
  DO K = KT+1,KB(I)-1
	...
	UDR		= (1.0D0+SIGN(1.0,U(K,I)))*0.5D0
	UDL		= (1.0D0+SIGN(1.0,U(K,I-1)))*0.5D0
	...
	ELSE
	  ATA		  = (1.0+SIGN(1.0,W(K-1,I)))*0.5D0
	  AB		  = (1.0+SIGN(1.0,W(K,I)))*0.5D0
...
use EDDY
use LOGICC
USE MAIN, ONLY:WARNING_OPEN; USE SCREENC, ONLY:JDAY
IMPLICIT NONE
...
INTEGER :: K,MULTIPLIER
    ...
    IF(STRICKON(JW))THEN
      ...
      IF(UST*KS/V>1.0)THEN
        BS1 = (5.5D0+2.5D0*LOG(UST*KS/V))*EXP(-0.217D0*LOG(UST*KS/V)**2)+8.5D0*(1-EXP(-0.217D0*LOG(UST*KS/V)**2))
