SUBROUTINE OUTPUTA
  ...
  ! *** DSI W2_TOOL LINKAGE
  Integer(real32),SAVE,ALLOCATABLE,DIMENSION(:)::WSEL
  Integer(real32),SAVE,ALLOCATABLE,DIMENSION(:,:)::WDSI
    ...
    IF (JDAY.GE.NXTBIO) THEN
      ...
      DO J=1,NIBIO
        ...
        DO K = KTWB(JW),KBI(I)
          ...
          END DO
	      WRITE(BIOEXPFN(J),'(F8.2,",",I8,",",3(F8.2,","))', advance='NO') JDAY, IBIO(J), DEPTHM(K,I), T1(K,I), GAMMA(K,I)
	      WRITE(BIOEXPFN(J),'(*(F8.3,","))', advance='NO') (C2ZOO(K,I,JC), JC = NZOOS,NZOOE)
...
3312            FORMAT(5X,'Macrophyte spec ',i2,/7X,'Spatially integrated mass [MACMBRS] = ',1PE15.8E2,1X,'g ',/7X,   &
    ...
    IF (PROFILE(JW)) THEN
      IF (JDAY >= NXTMPR(JW) .OR. JDAY >= PRFD(PRFDP(JW)+1,JW)) THEN
        ...
        WRITE (PRF(JW),'(F8.3," ",A3,I3,A,2I4,F8.4,I8)')JDAY,ADJUSTL(MONTH),GDAY,', ',YEAR,KTWB(JW), REAL(Z(DS(BS(JW)))), NSPRF(JW)
        DO JP=1,NIPRF(JW)
    ...
    IF (FLUX(JW)) THEN
      IF (JDAY >= NXTMFL(JW) .OR. JDAY >= FLXD(FLXDP(JW)+1,JW)) THEN
        ...
        ENDIF
          ...
          NEW_PAGE = NLINES > 72
          WRITE (FLX(JW),'(/A,F10.3," ",3(A,I0),A,F0.2,A/)') 'New date ', JDAY, MONTH//' ', GDAY, ', ', YEAR, '   Julian Date = ',    &
          INT(JDAY),' days ',(JDAY-INT(JDAY))*24.0,                            &
          ' hours           '//KFNAME(KFCN(JAF,JW))
          WRITE (FLX(JW),'("   ",2000I10)')                  (ISNP(I,JW),I=1,NISNP(JW))
          DO K=KTWB(JW),KBR(JW)
            WRITE (FLX(JW),'(" ",I2,200A)') K,(CONV(K,I),I=1,NISNP(JW))
          END DO
        END DO
